...
#ifndef MVCCART_ARTCPP_H
#define MVCCART_ARTCPP_H
...
class ArtCPP {
     ...
     * @return 0 on success.
     */
    private: int art_tree_init(std::shared_ptr<art_tree> t) {
        t->root = NULL;
        t->size = 0;
        return 0;
    }
    ...
    public:  static void destroy_node(art_node *n) {
        // Break if null
        if (!n) return;
     ...
     * @return 0 on success.
     */
    public: int art_tree_destroy(std::shared_ptr<art_tree> t) {
        destroy_node(t->root);
        return 0;
    }
...
// Simple inlined if
     ...
     * the key and node.
     */
    public:  static int check_prefix(const art_node *n, const unsigned char *key, int key_len, int depth) {
     ...
     * @return 0 on success.
     */
    public:  static int leaf_matches(const art_leaf *n, const unsigned char *key, int key_len, int depth) {
        (void)depth;
     ...
     */
    public: void* art_search(const unsigned char *key, int key_len) {
            art_node **child;
            art_node *n = t->root;
     ...
     */
    public: art_leaf* art_minimum(art_tree *t) {
     ...
     */
    public: art_leaf* art_maximum(art_tree *t) {
     ...
     */
    public:void* art_insert(const unsigned char *key, int key_len, void *value)
    {
        int old_val = 0;
        void *old = recursive_insert(this->t->root, &t->root, key, key_len, value, 0, &old_val);
        if (!old_val) t->size++;
        return old;
    }
